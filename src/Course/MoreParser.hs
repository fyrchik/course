...
betweenCharTok ::
  ...
  -> Parser a
  -> Parser a
betweenCharTok lc rc p =
  between (charTok lc) (charTok rc) p
...
hex =
  ...
  let Full c = readHex t in valueParser (chr c) -- readHex always succeed actually
...
satisfyAll ::
  ...
  -> Parser Char
satisfyAll ps =
  satisfy (\t -> and (map ($ t) ps)) -- see satisfyAny
...
satisfyAny ::
  ...
  -> Parser Char
satisfyAny ps =
  satisfy (or . sequence ps) -- satisfyAll can also be written like this
